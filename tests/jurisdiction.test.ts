...
describe('jurisdiction validation', () => {
  test('accepts GB', () => {
    ...
    expect(result).toEqual({ valid: true, jurisdiction: 'FR' });
  });
  ...
  test('rejects unsupported jurisdiction', () => {
    ...
    expect(result).toEqual({
      ...
      error: {
        ...
        supported: ['FR'],
        message: 'This server currently covers France. More jurisdictions are planned.',
      },
    });
  });
  ...
  test('SUPPORTED_JURISDICTIONS contains GB', () => {
    expect(SUPPORTED_JURISDICTIONS).toContain('FR');
  });
});
